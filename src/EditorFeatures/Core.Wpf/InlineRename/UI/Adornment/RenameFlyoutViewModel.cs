...
#pragma warning restore CS0618 
        ...
        public async Task SubmitAsync()
        {
            ...
            try
            {
                ...
                await Session.CommitAsync(previewChanges: false).ConfigureAwait(false);
            }
            catch (Exception ex) when (FatalError.ReportAndCatch(ex, ErrorSeverity.Critical))
            {
                // Show a nice error to the user via an info bar
                var errorReportingService = Session.Workspace.Services.GetRequiredService<IErrorReportingService>();
